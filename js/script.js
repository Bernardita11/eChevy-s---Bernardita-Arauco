...
function updateHeroTitle(titleIndex) {
    let txt1 = `Los mejores frutos secos para la salud de tu familia`;
    let txt2 = `Satisfacer tus necesidades diarias de forma fácil e instantánea`;
    let txt3 = `Prueba frutas secas para un mejor estilo de vida`;
    let txt4 = `Ten hasta un 50% de descuento en todos los productos`;
    let txt5 = `Ofertas especiales disponibles con productos increíbles`;
    let txt6 = `Verduras frescas con un gran descuento`;
...
function displayBuyingHeader(countValue) {
    ...
    if (countValue > 0 && isSelectedItemActive === true) {
        buyingProductContent.classList.add('active-buying-title');
    } else if (totalShopItems > 0 && isSelectedItemActive === true) {
        buyingProductContent.classList.add('active-buying-title');
    } else {
        buyingProductContent.classList.remove('active-buying-title');
    }
}
...
function createSelectedProductsContent(image, name, price, unit, discount, preservative, time) {
    ...
    newStrong[0].textContent = 'Producto: ';
    newStrong[1].textContent = 'Precio: ';
    newStrong[2].textContent = 'Descuento: ';
    newStrong[3].textContent = 'Cantidad: ';
    newStrong[4].textContent = 'Conservantes: ';
    newStrong[5].textContent = 'Tiempo añadido: ';
    ...
    newShoppingButton[0].textContent = 'Agregar al Carrito';
...
function removeItemsToSelectedProducts(productIndex) {
    ...
    addToCartBtn[productIndex].innerHTML = '<span class="icon-cart-plus"></span> Añadir al Carrito';
...
function deactiveFavoriteItemAddToCartBtn(productIndex) {
    ...
    favoriteItemAddToCartBtn.innerHTML = '<span class="icon-cart-plus"></span> Añadir al Carrito';
}
...
function controlFavoriteProductItems(itemIndex) {
    ...
    favoriteContent.addEventListener('click', function () {
        activeConfirmationBox('Remover producto de Favoritos?');
    ...
    // select 'Añadir al Carrito' button of favorite item
    let favoriteItemAddToCartBtn = newfavoriteItem[itemIndex].children[2].children[2].children[0];

    // actions while click 'Añadir al Carrito' button of favorite item 
...
removeAllShopItems.onclick = () => {
    if (countAddToBuyItem > 0) {
        activeConfirmationBox('Remover todos los productos del carrito?');
        removeConfirmBtn.onclick = () => {
            for (let i = 0; i < storeShopItemsIndex.length; i++) {
    ...
    for (let i = 0; i < featuredProducts.length; i++) {
        ...
        if (productClass === freaturedProductClass) {
            if (addedToCart[i] === false) {
                notificationMessage.textContent = `Estás seguro de agregar este producto al carrito?`;
                ...
                notificationMessage.textContent = `¡Este producto yae stá en el carrito! Quiere removerlo?`;
    ...
    for (let i = 0; i < featuredProducts.length; i++) {
        ...
        if (productClass === freaturedProductClass) {
            if (addedToFavorite[i] === false) {
                notificationMessage.textContent = `¿Estás seguro de añadir este producto en favoritos?`;
                notificationBoxConfirmBtn.textContent = `Ok`;
                ...
                notificationMessage.textContent = `¡Este producto ya está en Favoritos! ¿Quiere removerlo?`;
                notificationBoxConfirmBtn.textContent = `Remove`;
